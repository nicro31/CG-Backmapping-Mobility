...
#define repetition 8
#define dummyNumberDouble 666.0

#define watch(x) std::cout << (#x) << " is " << (x) << std::endl;

// Typedef to implement easy switch between double/long double
using doubleType = long double;

enum Direction {x, y, z};

enum AtomType { C, O, S, H, N, Z};

enum FragmentType { electron, hole, neutral};
...
struct RigidFragment
{
    ...
    void orientation(const std::vector<Vect3> &inputPoints, Vect3 &center, Vect3 &orientP, Vect3 &orientN, Vect3 &orientT);
