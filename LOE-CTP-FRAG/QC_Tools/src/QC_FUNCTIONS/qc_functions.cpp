...
vector<double> calculate_transfer_integral(
  Matrix mat_1_Coef,
  Matrix mat_2_Coef,
  Matrix mat_P_Coef,
  Matrix mat_S,
  ...
  //Matrix S_BA = gammaA * gammaB_inv;

  Matrix Energy = Matrix_diag( mat_P_OE );
  Matrix& J_AB = gammaB * (Energy * gammaA_inv);
  //Matrix J_BA = gammaA * (Energy * gammaB_inv);

  Matrix& e_B = gammaB * (Energy * gammaB_inv );
  Matrix& e_A = gammaA * (Energy * gammaA_inv );

  double J_ab = J_AB.get_elem(1,1);
  //double J_ba = J_BA.get_elem(1,1);
  double e_b = e_B.get_elem(1,1);
  double e_a = e_A.get_elem(1,1);
  double S_ab = S_AB.get_elem(1,1);
  //double S_ba = S_BA.get_elem(1,1);

  double J_eff = (J_ab-1/((double)2)*(e_b+e_a)*S_ab);
  J_eff = J_eff/((double)(1-pow(S_ab,2)));
  
  //double scaling_a( (1.0/(double)2.0) * 1.0/((double)(1-pow(S_ab,2))) );
  //double scaling_b( (1.0/(double)2.0) * 1.0/((double)(1-pow(S_ba,2))) );
  //double e_a_eff( scaling_a * ( ( e_a + e_b ) - 2*J_ab*S_ab + (e_a-e_b)*sqrt(1-pow(S_ab,2)) ) );
  //double e_b_eff( scaling_b * ( ( e_b + e_a ) - 2*J_ba*S_ba + (e_b-e_a)*sqrt(1-pow(S_ba,2)) ) );


  //cout << "J_ab  " << J_ab*hartreeToeV << " eV\n";
  //cout << "J_ba  " << J_ba*hartreeToeV << " eV\n";
  //cout << "e_a   " << e_a*hartreeToeV << " eV\n";
  //cout << "e_b   " << e_b*hartreeToeV << " eV\n";
  //cout << "S_ab  " << S_ab << "\n";
  //cout << "S_ba  " << S_ba << "\n";
  //cout << "e_a_eff   " << e_a_eff*hartreeToeV << " eV\n";
  //cout << "e_b_eff   " << e_b_eff*hartreeToeV << " eV\n";
  //cout << "J_eff " << J_eff*hartreeToeV << " eV\n";

  ///// N .Rolland Modif to have access to the calculation results and to calculate effective energies /////
  //return J_eff;

  vector<double> result;
  result.push_back(e_a*hartreeToeV);
  result.push_back(e_b*hartreeToeV);
  result.push_back(S_ab);
  result.push_back(J_ab*hartreeToeV);
  result.push_back(J_eff*hartreeToeV);
  
  
  
  /// N. ROLLAND Take care of memory leaks ///
  delete &Inter;
  delete &gammaA;
  delete &gammaB;
  delete &S_AB;
...
void freeListMatrix(list<Matrix *> &lm)
{
    for(list<Matrix *>::iterator it = lm.begin(); it!=lm.end(); ++it)
    {
        if(*it != nullptr){ delete *it; *it = nullptr;}
    }  
}
...
list<Matrix *> splitMatrixIntoList(
  ...
  if(ColRowSplit.compare("Columns")==0){
    ...
    for(auto i=1; i<=num_sub_matrices;++i){
      Matrix * mat_new = new Matrix(mat->get_rows(),subMatrixDimension.at(i-1));
    ...
    int row = 0;
    for(auto i=1; i<=num_sub_matrices;++i){
      Matrix * mat_new = new Matrix(subMatrixDimension.at(i-1),mat->get_cols());
...
list<Matrix *> splitCoefsUpByAtoms(vector<int> basisFuncP, Matrix * Coefs,string ColRow){
...
void refreshSwapOrder(vector<pair<int,int>> & monBmatch, vector<pair<int,int>>& monAmatch){
...
Matrix * mergeListOfMatrices(
  ...
  if(ColRowMerge.compare("Columns")==0){
    ...
    for( auto it=matrix_list.begin();it!=matrix_list.end();++it){
      Matrix * mat_ptr = *it;
      int row = 1;
      if(col>cols) throw runtime_error("Your new matrix is not large enough");
      for( auto i=1;i<=mat_ptr->get_rows();++i){
        if(row>rows) throw runtime_error("Your new matrix is not large enough");
...
Matrix * unscramble_Coef(
  ...
  list<Matrix *> p_atom_mat_coef = splitCoefsUpByAtoms(basisFuncP,dimerCoef,"Columns");
  ...
  Matrix * dimerCoef_new = createNewMatrix(p_atom_mat_coef,dimerCoef->get_rows(),dimerCoef->get_cols(),"Columns");
  
  //N. ROLLAND memory leak
  freeListMatrix(p_atom_mat_coef);
...
vector<double> TransferComplex::calcJ(map<string,string> orbitaltype, map<string,int> orbnum){
