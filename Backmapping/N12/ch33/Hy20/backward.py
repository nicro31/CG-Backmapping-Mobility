...
AminoAcids    = "ALA CYS ASP GLU PHE GLY HIS ILE LYS LEU MET ASN PRO GLN ARG SER THR VAL TRP TYR ACE NH2 PDT".split()
...
# Add by Nicolas Rolland, to manually add water with genbox later on
counterSol = 0
waterFile = open('nbWater.txt','w')
...
# Add by Nicolas Rolland, to manually add water with genbox later on
waterFile.write(str(counterSol))
waterFile.close()
...
if options["-p"] and options["-po"]:
    ...
    # Modification by Nicolas Rolland, to introduce solvent with gromacs genbox
    po.writelines(["%s %5d\n"%(a,len(list(b))) for a,b in itertools.groupby(sol)])
    if not options["-sol"]:
        solName = 'SOL'
        po.writelines("%s %5d\n"%(solName,4*counterSol))
...
if options["-n"]:
    ...
    # Add solvent, modif. by Nicolas Rolland
    if not options["-sol"]:
        for i in range(3*4*counterSol):
            ndx_solvent.append(ndx_membrane[-1]+i+1)
